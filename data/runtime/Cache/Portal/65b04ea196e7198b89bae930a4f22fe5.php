...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="javascript:;">所有资源</a></li>
			<li><a href="<?php echo U('AdminPost/add',array('orgid'=>$orgid,'term'=>empty($term['term_id'])?'':$term['term_id']));?>" target="_self">添加资源</a></li>
		</ul>
		<form class="well form-search" method="post" action="<?php echo U('AdminPost/index',array('orgid'=>$orgid));?>">
		...
		<form class="J_ajaxForm" action="" method="post">
			...
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($posts)): foreach($posts as $key=>$vo): ?><tr>
					...
					<td><?php echo ((isset($users[$vo['post_author']]['nicename']) && ($users[$vo['post_author']]['nicename'] !== ""))?($users[$vo['post_author']]['nicename']):$users[$vo['post_author']]['user_login']); ?></td>
					...
					<td>
						<!-- <a href="<?php echo U('AdminPost/edit',array('orgid'=>$orgid,'term'=>empty($term['term_id'])?'':$term['term_id'],'id'=>$vo['id']));?>">修改</a> |  -->
			...
			Wind.use('ajaxForm', 'artDialog', 'iframeTools', function() {
				...
				$('.J_articles_move').click(
						function(e) {
							...
							var $this = $(this);
							art.dialog.open(
									"/thinkcmfx/index.php?g=portal&m=AdminPost&a=move&ids="
											+ id, {
										title : "批量移动",
										width : "80%"
									});
