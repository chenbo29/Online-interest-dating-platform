...
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="<?php echo U('user/index',array('orgid'=>$orgid));?>">管理员</a></li>
		...
		<table class="table table-hover table-bordered">
			...
			<tbody>
				...
				<?php if(is_array($users)): foreach($users as $key=>$vo): ?><tr>
					...
					<td><?php echo ($user_statuses[$vo['identify']]); ?></td>
					<td>
						<?php if($vo['identify'] == 1): ?><font color="#cccccc">修改</font> | <font color="#cccccc">删除</font> |
							<?php if($vo['identify'] == 1): ?><font color="#cccccc">停用</font>
							<?php else: ?>
								<font color="#cccccc">启用</font><?php endif; ?>
						<?php else: ?>
							<a href='<?php echo U("user/edit",array("uid"=>$vo["user_id"],"orgid"=>$orgid));?>'>修改</a> | 
							<a class="J_ajax_del" href="<?php echo U('user/delete',array('identify'=>$vo['identify'],'uid'=>$vo['user_id'],'orgid'=>$orgid));?>">删除</a> | 
							<?php if($vo['identify'] == 2): ?><a href="<?php echo U('user/ban',array('id'=>$vo['id']));?>" class="J_ajax_dialog_btn" data-msg="您确定要停用此用户吗？">停用</a>
							<?php else: ?>
