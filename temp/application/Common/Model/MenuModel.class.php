...
class MenuModel extends CommonModel {
     ...
     */
    public function admin_menu($parentid, $with_self = false,$orgid) {
        //父节点ID
        ...
        foreach ($result as $v) {
            ...
            if (preg_match('/^public_/', $action)) {
                ...
                if ( sp_auth_check(sp_get_current_admin_id(),$rule_name,'',$orgid)){//检查用户是否有该应用权限
                	$array[] = $v;
                }
     ...
     * 菜单树状结构集合
     */
    public function menu_json($orgid=null) {
        if($orgid==null){
            return false;
        }
        $data = $this->get_tree(0,'','',$orgid);
        return $data;
    }

    /*取得树形结构的菜单*/
    public function get_tree($myid, $parent = "", $Level = 1,$orgid=null) {
        /*if($orgid==null){return false;}*/
        $data = $this->admin_menu($myid,'',$orgid);
        $Level++;
        if (is_array($data)) {
            foreach ($data as $a) {
                $id = $a['id'];
              ...
              $fu = "";
                ...
                $array = array(
                    ...
                    "name" => $a['name'],/*菜单名称*/
                    "parent" => $parent,
                    "url" => U("{$name}/{$model}/{$action}{$fu}", array("menuid" => $id,'orgid'=>$orgid)),
                ); 
                
                
                
                $ret[$id . $name] = $array;
                $child = $this->get_tree($a['id'], $id, $Level,$orgid);
