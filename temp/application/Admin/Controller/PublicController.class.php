            ...
            */
    		...
    		if(!empty($result) && $result['identify']!=3 ){
    			...
    			if($result['user_pass'] == sp_password($pass)){//密码正确
    				...
    				//获取用户角色并检查角色是否被禁用
    				$groups=$role_user_model->alias("a")->join($role_user_join)->where(array("user_id"=>$result["user_id"],"status"=>1))->getField("role_id",true);//连接角色表和用户角色表，通过用户id查找该用户都有哪些权限
    				//角色切没有被禁用,且用户也没有被禁用
    				...
    				//登入成功页面跳转
    				$_SESSION["ADMIN_ID"]=$result["user_id"];//保存管理员用户名，id
    				$_SESSION['name']=$result["user_login"];
    				$data['last_ip']=get_client_ip();
    				$data['last_login_time']=time();//date("Y-m-d H:i:s");
    				//$org_user_viewmodel->where($where)->save($data);
                    $user_org_model=M("UserOrg");
                    $where1['id']=$result['id'];
                    $user_org_model->where($where1)->save($data);
    				setcookie("admin_username",$name,time()+30*24*3600,"/");//设置cookis
    				$this->success(L('LOGIN_SUCCESS'),U("Index/index",array("orgid"=>$orgid)));//传递组织id
    			}else{
