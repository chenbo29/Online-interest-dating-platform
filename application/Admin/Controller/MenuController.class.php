...
class MenuController extends AdminbaseController {
    	...
    	if (IS_POST) {
    		if ($this->menu_model->create()) {
    			if ($this->menu_model->add()!==false) {
    				...
    				/*如果没有这个方法，在auth_rule表里插入一条数据,应该保持主键id一致*/
    ...
    public function delete() {
        ...
        if ($this->menu_model->delete($id)!==false) {
            /*同步删除auth_rule表*/
            $find_rule=$this->auth_rule_model->delete($id);
    	...
    	if (IS_POST) {
    		if ($this->menu_model->create()) {
    			if ($this->menu_model->save() !== false) {
    				...
    				$mwhere=array("name"=>$name);
                    $id=I('post.id');
                    $where['id']=$id;/*auth_rule 和menu表同步*/
    				/*如果我修改的是方法，那么auth_rule表新添加一条记录，不合理？ by liu*/
    				/*$find_rule=$this->auth_rule_model->where($mwhere)->find();*//*by liu*/
                    $find_rule=$this->auth_rule_model->where($where)->find();
    				if(!$find_rule){
                        /*修改时为何是添加 by liu*/
    					$this->auth_rule_model->add(array("name"=>$name,"module"=>$app,"type"=>"admin_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}else{
    					$this->auth_rule_model->where($where)->save(array("name"=>$name,"module"=>$app,"type"=>"admin_url","title"=>$menu_name));//type 1-admin rule;2-user rule
    				}
