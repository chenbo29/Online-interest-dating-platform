...
class LoginController extends HomeBaseController {
	...
	function index(){
	    if(sp_is_user_login()){ //已经登录时直接跳到首页
	        redirect(__ROOT__."/index.php/User/St/likest");
...
/*暂时注释验证码*/
     ...
     //        $this->Ajaxreturn($info);
        ...
        extract($_POST);/*跟据前台表单标签name实例化变量*/
        if(empty($username)){
            $info=array(
                'statu'=>-3,
                'info'=>"用户名或者邮箱不能为空！",
    	...
    	if($result != null)
    	{
    		if($result['user_pass'] == sp_password($password)|| $ucenter_login_ok)
    		{
    			...
    			$redirect=empty($_SESSION['login_http_referer'])?__CONTROLLER__."/myindex":$_SESSION['login_http_referer'];
