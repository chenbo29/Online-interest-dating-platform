...
class UserPostController extends MemberbaseController {
	...
	function _initialize() {
		// parent::_initialize();
		$this->posts_model = D("Common/Tieba");
	...
	function add_post(){
		...
		if (IS_POST) {
			$article['post_title']=trim($_POST['title']);
			$article['post_content']=trim($_POST['post_content']);

			if($article['post_title']==='')
			{
				$this->error("请填写标题！");
			}
			elseif ($article['post_content']==='') {
				$this->error("请填写内容！");
			}

			$article['ORG_ID']=$_REQUEST['stid'];
			$article['post_date']=date("Y-m-d H:i:s",time());
			$article['post_modified']=$article['post_date'];
			$article['post_author']=get_current_userid();
			$article['post_excerpt']=msubstr(strip_tags($article['post_content']),0,256);
			// dump($article['post_excerpt']);die;
			$result=$this->posts_model->add($article);
			// dump($this->posts_model->_sql());die;
			if ($result) {
				$this->success("发帖成功！");
			} else {
				$this->error("发帖失败！");
