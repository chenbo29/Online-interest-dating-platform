...
class UeditorShareController extends Controller {
	...
	function upload(){
		...
		switch ($action) {
			case 'config':
				...
				/* 上传图片 */
			case 'uploadimage':
				$result = $this->_ueditor_upload(array('maxSize' =>2097152),$orgid);
				break;
				/* 上传涂鸦 */
			case 'uploadscrawl':
	...
	private function _ueditor_upload($config=array(),$orgid){
		...
		if ($info) {
			...
			if(!empty($info['upfile']['url'])){
				...
				$url = C("TMPL_PARSE_STRING.__UPLOAD__").$typeconfig['savePath'].$info['upfile']['savename'];
			}
