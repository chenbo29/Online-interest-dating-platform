...
class UeditorPastactivityController extends Controller {
	...
	function upload(){
		...
		switch ($action) {
			case 'config':
				...
				/* 上传图片 */
			case 'uploadimage':
				$result = $this->_ueditor_upload(array('maxSize' =>2097152)/*2M*/,$orgid);
				break;
				/* 上传涂鸦 */
			case 'uploadscrawl':
				$result = $this->_ueditor_upload('',$orgid);
				break;
				/* 上传视频 */
			case 'uploadvideo':
				$result = $this->_ueditor_upload(array('maxSize' =>20971520 ,/*20M 1073741824 1G*/'exts'  =>    array('mp4', 'avi', 'wmv','rm','rmvb','mkv')),$orgid);
				break;
				/* 上传文件 */
			case 'uploadfile':
				$result = $this->_ueditor_upload(array('maxSize' =>5242880)/*5M*/,$orgid);
				break;
	...
	private function _ueditor_upload($config=array(),$orgid){
		
		$date=date("Ymd");
		/*上传处理类*/
		$mconfig=array(
				'rootPath' => './'. C("UPLOADPATH"),
				'savePath' => "activityshow/".$orgid.'/',
				'maxSize' => 10485760,/*10M*/
				'saveName'   =>    array('uniqid',''),
				/*'exts'       =>    array('jpg', 'gif', 'png', 'jpeg'),*/
				'autoSub'    =>    false,
		);
		// $yswj=".rar.zip.tar.gz.7z.bz2.cab.iso";/*压缩文件*/
		// $document=".doc.docx.xls.xlsx.ppt.pptx.pdf.txt.md.xml";/*文档文件*/
		// $picture=".png.jpg.f4v.jpeg.gif.bmp";/*图片文件*/
		// //$type=$_FILES['upfile']['type'];
		// $name=$_FILES['upfile']['name'];
		// /*获取文件后缀名*/
		// $position=strripos($name,'.');//
  //       $type=substr($name,$position);
  //       /*根据不同类型文件上传不同文件夹,为每个组织创建一个文件夹*/
		// if(stripos($picture,$type)!==false){/*图片文件*/
		// 	$orgpath="__ROOT__/data/upload/share/picture/".$orgid;
		// 	if(!file_exists($orgpath)){
		// 		mkdir($orgpath);
		// 	}
		// 	$spath="share/picture/".$orgid."/";
		// 	$typeconfig=array('savePath'=>$spath);
		// 	/*$typeconfig=array('savePath'=>"share/picture/");*/
		// 	$mconfig=array_merge($mconfig,$typeconfig);
		// }elseif(stripos($document,$type)!==false){/*文档文件*/
		// 	$orgpath="__ROOT__/data/upload/share/document/".$orgid;
		// 	if(!file_exists($orgpath)){
		// 		mkdir($orgpath);
		// 	}
		// 	$spath="share/document/".$orgid."/";
		// 	$typeconfig=array('savePath'=>$spath);			
		// 	/*$typeconfig=array('savePath'=>"share/document/");*/
		// 	$mconfig=array_merge($mconfig,$typeconfig);
		// }elseif(stripos($yswj,$type)!==false){/*压缩文件*/
		// 	$orgpath="__ROOT__/data/upload/share/zip/".$orgid;
		// 	if(!file_exists($orgpath)){
		// 		mkdir($orgpath);
		// 	}
		// 	$spath="share/zip/".$orgid."/";	
		// 	$typeconfig=array('savePath'=>$spath);			
		// 	/*$typeconfig=array('savePath'=>"share/zip/");*/
		// 	$mconfig=array_merge($mconfig,$typeconfig);

		// }else{/*其他文件*/
		// 	$orgpath="__ROOT__/data/upload/share/other/".$orgid;
		// 	if(!file_exists($orgpath)){
		// 		mkdir($orgpath);
		// 	}
		// 	$spath="share/other/".$orgid."/";
		// 	$typeconfig=array('savePath'=>$spath);		
		// 	/*$typeconfig=array('savePath'=>"share/other/");*/
		// 	$mconfig=array_merge($mconfig,$typeconfig);
		// }
		$orgpath="__ROOT__/data/upload/activityshow/".$orgid;
			if(!file_exists($orgpath)){
				mkdir($orgpath);
			}
		if(is_array($config)){
		...
		if ($info) {
			...
			if(!empty($info['upfile']['url'])){
				...
				$url = C("TMPL_PARSE_STRING.__UPLOAD__").$mconfig['savePath'].$info['upfile']['savename'];
			}
