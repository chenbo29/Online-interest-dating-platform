...
class AdminbaseController extends AppframeController {
    ...
    function _initialize(){
       ...
       if(!isset($_SESSION['ADMIN_ID'])){
        ...
        /*$orgid=2;*/
        /*是否进行后台登陆就这这里判断*/
    	/*if(isset($_SESSION['ADMIN_ID']) && !empty($orgid)){
    		$users_obj= M("Users");
    		$id=$_SESSION['ADMIN_ID'];
    		$user=$users_obj->where("id=$id")->find();
    		if(!$this->check_access($id,$orgid)){//检查权限时加上组织id by liu
    			$this->error("您没有访问权限！");
    			exit();
    		}
    		...
    		if(IS_AJAX){
    			$this->error("您还没有登录！",U("admin/public/login",array("orgid"=>$orgid)));
    		}else{
    			header("Location:".U("admin/public/login",array("orgid"=>$orgid)));
    			exit();
    		}
    ...
    private function check_access($uid,$orgid){
    	...
    	if(MODULE_NAME.CONTROLLER_NAME.ACTION_NAME!="AdminIndexindex"){
    		return sp_auth_check($uid,'','',$orgid);
    	}else{
    		return true;
    	}
    }
